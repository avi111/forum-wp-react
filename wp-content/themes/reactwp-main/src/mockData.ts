...
במקביל להחלשות ה-DMN, אנו רואים עליה דרמטית ב"קישוריות התפקודית הגלובלית" (Global Functional Connectivity). אזורים במוח שבדרך כלל אינם מתקשרים זה עם זה ישירות, מתחילים "לדבר". כך למשל, הקורטקס הראייתי עורף לתקשר ישירות עם אזורים האחראיים על עיבוד רגשי או שמיעתי, מה שעשוי להסביר תופעות כמו סינסתזיה (עירוב חושי).
...
export const MOCK_STUDENT_PAPERS: StudentPaper[] = [
  {
    id: "1",
...
export const MOCK_STUDENT_JOBS: StudentJob[] = [
  {
    ...
    applyLink: "mailto:jobs@example.ac.il",
  },
  {
    id: "j2",
...
export const MOCK_QUESTIONNAIRES: Questionnaire[] = [
  ...
  },
  {
    ...
    `,
    imageUrl:
      "https://fastly.picsum.photos/id/1000/800/534.jpg?hmac=4QlxdayOHzplv6qf482AP8zKetrQTUfkw54eDaRLKrI",
    authorId: "res1", // Assign an author ID
  },
];

export const MOCK_IMAGING_METHODS: ImagingMethod[] = [
  {
    id: "im1",
    ...
    fullContent: `
      <h2>מהו fMRI?</h2>
      <p>דימות תהודה מגנטית תפקודי (fMRI) היא טכניקה לא פולשנית המודדת שינויים בזרימת הדם במוח, המקושרים לפעילות עצבית. כאשר אזור מסוים במוח פעיל יותר, זרימת הדם אליו גוברת, ושינוי זה נקלט על ידי המכשיר.</p>
      <h3>עקרונות פעולה:</h3>
      <ul>
        <li>המכשיר מזהה שינויים ברמת החמצון של המוגלובין (BOLD signal).</li>
        <li>רזולוציית זמן טובה יחסית (שניות) ורזולוציית מרחב גבוהה (מילימטרים).</li>
      </ul>
      <h3>שימושים במחקר פסיכדלי:</h3>
      <p>fMRI מאפשר לחוקרים למפות אילו אזורים במוח מופעלים או מושתקים תחת השפעת חומרים פסיכדליים, ולבחון שינויים בקישוריות בין אזורים שונים. לדוגמה, מחקרים הראו הפחתה בפעילות רשת ברירת המחדל (DMN) תחת השפעת פסילוסיבין.</p>
    `,
  },
  {
    id: "im2",
    ...
    fullContent: `
      <h2>מהו EEG?</h2>
      <p>אלקטרואנצפלוגרפיה (EEG) היא שיטה המודדת פעילות חשמלית במוח באמצעות אלקטרודות המוצמדות לקרקפת. היא מספקת מידע על שינויים מהירים בפעילות המוחית.</p>
      <h3>עקרונות פעולה:</h3>
      <ul>
        <li>מזהה פוטנציאלים חשמליים הנוצרים על ידי נוירונים.</li>
        <li>רזולוציית זמן מצוינת (מילישניות), אך רזולוציית מרחב נמוכה.</li>
      </ul>
      <h3>שימושים בממחקר פסיכדלי:</h3>
      <p>EEG משמש לחקר גלי מוח שונים (אלפא, בטא, תטא, דלתא) תחת השפעת פסיכדליים, ומאפשר לזהות שינויים במצבי תודעה. לדוגמה, שינויים בגלי אלפא מקושרים למצבי רגיעה ושינוי תפיסתי.</p>
    `,
  },
  {
    id: "im3",
    ...
    fullContent: `
      <h2>מהו PET?</h2>
      <p>טומוגרפיית פליטת פוזיטרונים (PET) היא טכניקת דימות גרעינית המשתמשת בחומרים רדיואקטיביים (טרייסרים) כדי להדגים תהליכים מטבוליים, זרימת דם, ופעילות רצפטורים במוח.</p>
      <h3>עקרונות פעולה:</h3>
      <ul>
        <li>הטרייסר נקשר למולקולות ספציפיות (למשל, רצפטורים לסרוטונין).</li>
        <li>מזהה פליטת פוזיטרונים מהטרייסר וממיר אותה לתמונה תלת-ממדית.</li>
      </ul>
      <h3>שימושים במחקר פסיכדלי:</h3>
      <p>PET חיוני לחקר צפיפות ופעילות רצפטורים ספציפיים במוח המושפעים מפסיכדליים, כמו רצפטור 5-HT2A, שהוא היעד העיקרי של פסיכדליים קלאסיים.</p>
    `,
  },
];

export const MOCK_RECRUITMENT_TOOLS: RecruitmentTool[] = [
  {
    id: "rec1",
    title: "פלטפורמת גיוס מתנדבים למחקר",
    excerpt: "מערכת לניהול וגיוס נחקרים פוטנציאליים למחקרים קליניים.",
    fullContent: `
      <h2>פלטפורמה לגיוס נחקרים</h2>
      <p>פלטפורמה זו מציעה פתרון מקיף לניהול תהליך גיוס המשתתפים למחקרים קליניים ואקדמיים. היא מאפשרת לחוקרים לפרסם קולות קורא, לסנן מועמדים על פי קריטריונים ספציפיים, ולנהל את התקשורת עם הנחקרים הפוטנציאליים.</p>
      <h3>תכונות עיקריות:</h3>
      <ul>
        <li>ממשק ידידותי לחוקרים ולנחקרים.</li>
        <li>כלי סינון מתקדמים.</li>
        <li>אפשרות לשליחת תזכורות ועדכונים אוטומטיים.</li>
        <li>הבטחת פרטיות ואבטחת מידע.</li>
      </ul>
      <p>השימוש בפלטפורמה מקצר משמעותית את זמן הגיוס ומגדיל את הסיכוי למצוא נחקרים מתאימים.</p>
    `,
  },
  {
    id: "rec2",
    ...
    fullContent: `
      <h2>הנחיות אתיות</h2>
      <p>מסמך זה מספק הנחיות מפורטות לגיוס נחקרים מאוכלוסיות פגיעות (כגון חולים פסיכיאטריים, קטינים, או אנשים עם מוגבלויות קוגניטיביות) למחקרים פסיכדליים. המטרה היא להבטיח את הגנתם, כבודם ורווחתם של המשתתפים.</p>
      <h3>נושאים הנסקרים:</h3>
      <ul>
        <li>הסכמה מדעת (Informed Consent) - דגשים מיוחדים.</li>
        <li>הגנה מפני ניצול.</li>
        <li>שמירה על סודיות ואנונימיות.</li>
        <li>תפקיד ועדת האתיקה (Helsinki Committee).</li>
      </ul>
      <p>אנו ממליצים לכל חוקר המעוניין לעבוד עם אוכלוסיות אלו לעיין במסמך זה בקפידה ולהתייעץ עם ועדת האתיקה הרלוונטית.</p>
    `,
  },
];
