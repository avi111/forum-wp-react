...
function iprf_convert_list_to_tailwind_steps($html)
{
    ...
    foreach ($list_items as $li) {
        ...
        $output .=
            '<span class="font-bold text-indigo-600">' . $counter . ".</span>";
...
{
    $template_name = isset($_POST["name"])
...
function iprf_fetch_researchers()
{
    ...
    if (!empty($user_query->get_results())) {
        foreach ($user_query->get_results() as $user) {
            ...
            $has_profile_picture = !empty(
                get_user_meta($user->ID, "wpcf-profile-image", true)
            );
            ...
            $researchers[] = [
                ...
                "institution" => !empty(
                    get_user_meta($user->ID, "wpcf-institution", true)
                )
                    ? get_user_meta($user->ID, "wpcf-institution", true)
                    : "N/A",
                "subSpecializations" => get_user_meta(
                    $user->ID,
                    "wpcf-sub-specializations",
                    false,
                ),
...
function iprf_fetch_events()
{
    ...
    $time_filter = isset($_POST["timeFilter"])
        ? sanitize_text_field($_POST["timeFilter"])
        : "all";
    ...
    while ($query->have_posts()) {
        ...
        $events[] = [
            ...
            "registrationLink" => iprf_get_field(
                get_the_ID(),
                "registration-link",
            ),
        ];
...
function iprf_fetch_trainings()
{
    ...
    while ($query->have_posts()) {
        ...
        $trainings[] = [
            ...
            "title" => get_the_title(),
            "description" => get_the_excerpt(),
            "fullDescription" => iprf_get_field(
                get_the_ID(),
                "full-description",
            ),
            ...
            "colorTheme" =>
                iprf_get_field(get_the_ID(), "color-theme") ?: "teal",
            "imageUrl" => get_the_post_thumbnail_url(get_the_ID(), "large"),
            "instructors" => explode(
                ",",
                iprf_get_field(get_the_ID(), "instructors"),
            ),
            "syllabus" => $syllabus,
        ];
    }
...
function iprf_subscribe_newsletter()
{
    ...
    if (class_exists(API::class)) {
        try {
            ...
            wp_send_json_error([
                "message" => "אירעה שגיאה בהרשמה. אנא נסה שנית מאוחר יותר.",
            ]);
        }
    } else {
...
function create_user_from_data($data)
{
    ...
    update_user_meta(
        $user_id,
        "wpcf-specialization",
        $data["mainSpecialization"],
    );

    // Assuming subSpecializations is an array
    if (
        isset($data["subSpecializations"]) &&
        is_array($data["subSpecializations"])
    ) {
        foreach ($data["subSpecializations"] as $sub_spec) {
            add_user_meta(
                $user_id,
                "wpcf-sub-specializations",
                sanitize_text_field($sub_spec),
            );
        }
    }
    ...
    foreach ($required_files as $file_key) {
        if (isset($_FILES[$file_key])) {
            ...
            if ($moved_file && !isset($moved_file["error"])) {
                ...
                return new WP_Error(
                    "upload_error",
                    "שגיאה בהעלאת קובץ: " . $moved_file["error"],
                );
            }
        }
    }
    ...
    if (
        isset($data["verificationDocUrl"]) &&
        !empty($data["verificationDocUrl"])
    ) {
    ...
    if (isset($uploaded_files["verificationDoc"])) {
        update_user_meta(
            $user_id,
            "wpcf-verification-doc-url",
            esc_url_raw($uploaded_files["verificationDoc"]),
        );
    }
    if (isset($uploaded_files["intentLetter"])) {
        update_user_meta(
            $user_id,
            "wpcf-intent-letter-url",
            esc_url_raw($uploaded_files["intentLetter"]),
        );
    }
    ...
    $message .=
        "ניתן לצפות בפרטי המשתמש ובמסמכים המצורפים בפרופיל המשתמש שלו בלוח הבקרה:\n";
...
function iprf_join_form_submit()
{
    ...
    if (
        !isset($_POST["agreedToBylaws"]) ||
        $_POST["agreedToBylaws"] !== "true"
    ) {
    ...
    // Verify reCAPTCHA
    $recaptcha_secret = defined("RECAPTCHA_SECRET_KEY")
    ...
    if ($recaptcha_secret) {
        ...
        if (!$result->success) {
            wp_send_json_error([
                "message" => "אימות ה-reCAPTCHA נכשל. אנא נסה שנית.",
            ]);
        }
    }
    ...
    $data = [
        ...
        "mainSpecialization" => sanitize_text_field(
            $_POST["mainSpecialization"],
        ),
        "subSpecializations" => isset($_POST["subSpecializations"])
            ? $_POST["subSpecializations"]
            : [],
    ...
    iprf_send_response([
        "message" => "ההרשמה בוצעה בהצלחה. פרטיך נשלחו לבדיקה ואישור.",
    ]);
}
...
add_action(
    "wp_ajax_nopriv_checkUserVerification",
    "iprf_check_user_verification",
);
...
function iprf_check_user_verification()
{
    ...
    echo '<!DOCTYPE html><html lang="he"><head><meta charset="UTF-8"><title>אימות משתמש</title>' .
        $style .
...
add_action(
    "wp_ajax_nopriv_processUserVerification",
    "iprf_process_user_verification",
);
...
function iprf_process_user_verification()
{
    ...
    $answer = isset($_POST["answer"])
        ? sanitize_text_field($_POST["answer"])
        : "";
    ...
    echo '<!DOCTYPE html><html lang="he"><head><meta charset="UTF-8"><title>תוצאת אימות</title>' .
        $style .
        '</head><body><div class="container">';
    ...
    if (!$user) {
        ...
        // Verify Name
        if (
            mb_strtolower(trim($answer), "UTF-8") ===
            mb_strtolower($user->first_name, "UTF-8")
        ) {
            // Update Role
 ...
 */
add_action(
    "wp_ajax_cleanupUnverifiedSubscribers",
    "iprf_cleanup_unverified_subscribers",
);
add_action(
    "wp_ajax_nopriv_cleanupUnverifiedSubscribers",
    "iprf_cleanup_unverified_subscribers",
);
...
function iprf_cleanup_unverified_subscribers()
{
    ...
    foreach ($users as $user) {
        ...
        if ($diff > $three_hours) {
            ...
            $message = "שלום,\n\n";
            $message .=
                "המשתמש שלך נמחק כיוון שלא אימתת אותו בחלון הזמן של 3 שעות שניתן לכך.\n";
            $message .=
                "אם ברצונך להירשם שוב, אתה מוזמן לשלוח מייל אל מנהלי האתר או לנסות להירשם שוב.\n\n";
            $message .= "בברכה,\nצוות האתר";
            $headers = ["Content-Type: text/plain; charset=UTF-8"];
    ...
    iprf_send_response([
        "message" => "תהליך ניקוי הסתיים. נמחקו $deleted_count משתמשים.",
    ]);
}
 ...
 */
add_action(
    "wp_ajax_fetchQuestionnairesByAuthor",
    "iprf_fetch_questionnaires_by_author",
);
add_action(
    "wp_ajax_nopriv_fetchQuestionnairesByAuthor",
    "iprf_fetch_questionnaires_by_author",
);
 ...
 */
add_action(
    "wp_ajax_fetchSingleStudentPaper",
    "iprf_fetch_single_student_paper",
);
add_action(
    "wp_ajax_nopriv_fetchSingleStudentPaper",
    "iprf_fetch_single_student_paper",
);
...
function iprf_fetch_single_student_paper()
{
    ...
    if (
        !$post ||
        $post->post_type !== "student-paper" ||
        $post->post_status !== "publish"
    ) {
        wp_send_json_error([
            "message" => "Student paper not found or not published",
        ]);
    }
...
add_action(
    "wp_ajax_nopriv_fetchSingleStudentJob",
    "iprf_fetch_single_student_job",
);
...
function iprf_fetch_single_student_job()
{
    ...
    if (
        !$post ||
        $post->post_type !== "student-job" ||
        $post->post_status !== "publish"
    ) {
        wp_send_json_error([
            "message" => "Student job not found or not published",
        ]);
    }
...
add_action(
    "wp_ajax_nopriv_fetchAllQuestionnaires",
    "iprf_fetch_all_questionnaires",
);
...
add_action(
    "wp_ajax_nopriv_fetchQuestionnaireById",
    "iprf_fetch_questionnaire_by_id",
);
...
function iprf_fetch_questionnaire_by_id()
{
    ...
    if (
        ...
        wp_send_json_error([
            "message" => "Questionnaire not found or not published",
        ]);
    }
...
add_action(
    "wp_ajax_nopriv_fetchAllImagingMethods",
    "iprf_fetch_all_imaging_methods",
);
...
add_action(
    "wp_ajax_nopriv_fetchImagingMethodById",
    "iprf_fetch_imaging_method_by_id",
);
 ...
 */
add_action(
    "wp_ajax_fetchAllRecruitmentTools",
    "iprf_fetch_all_recruitment_tools",
);
add_action(
    "wp_ajax_nopriv_fetchAllRecruitmentTools",
    "iprf_fetch_all_recruitment_tools",
);
 ...
 */
add_action(
    "wp_ajax_fetchRecruitmentItemById",
    "iprf_fetch_recruitment_item_by_id",
);
add_action(
    "wp_ajax_nopriv_fetchRecruitmentItemById",
    "iprf_fetch_recruitment_item_by_id",
);
...
function iprf_fetch_recruitment_item_by_id()
{
    ...
    if (
        !$post ||
        $post->post_type !== "recruitment-tool" ||
        $post->post_status !== "publish"
    ) {
